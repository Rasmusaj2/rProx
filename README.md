...
⚠️**Do not use third party hosts. These can be malicious and steal your account.**
It is recommended to run rProx locally, as this also reduces your latency.
...
Writing a plugin is simple. Add a file in the `./plugins/` directory (Or whats set in `pluginDirectory` in the config). Plugins support javascript code. An example is supplied in [`plugins/example.plugin.js`](plugins/example.plugin.js).